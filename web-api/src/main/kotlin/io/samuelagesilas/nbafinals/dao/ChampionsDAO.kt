...
interface ChampionsDAO {
    @SqlQuery("SELECT distinct year FROM champions order by year ASC")
    fun selectAllChampionshipYears(): List<Int>

    @SqlQuery("SELECT distinct team FROM champions order by team ASC")
    fun selectChampionshipTeams(): List<String>

    @SqlQuery("SELECT * FROM champions where year=:year order by game ASC")
    fun selectAllGamesByYear(@Bind("year") year: Int): List<ChampionsModel>

    @SqlQuery("SELECT * FROM champions where id=:id")
    fun selectGameById(@Bind("id") id: Int) : ChampionsModel?

    @SqlQuery("SELECT * FROM champions where year=:year AND win=1")
    fun selectAllGamesWonByYear(@Bind("year") year: Int): List<ChampionsModel>

    @SqlQuery("SELECT * FROM champions where year=:year AND win=0")
    fun selectAllGamesLostByYear(@Bind("year") year: Int): List<ChampionsModel>

    @SqlQuery("SELECT * FROM champions where year=:year AND home=1")
    fun selectAllHomeGamesByYear(@Bind("year") year: Int): List<ChampionsModel>

    @SqlQuery("SELECT * FROM champions where year=:year AND home=0")
    fun selectAllAwayGamesByYear(@Bind("year") year: Int): List<ChampionsModel>

    @SqlQuery("SELECT * FROM champions where team=:team")
    fun selectAllGamesByTeamName(@Bind("team") team: String): List<ChampionsModel>

    @SqlQuery("SELECT * FROM champions where team=:team and year=:year")
